...
| `UserIdentity`   | `auth`   | Authentication provider credentials (local password, OIDC sub)  | Masked (provider ID) | Contract lifetime                | 0.7           |
| `Session`        | `auth`   | Active refresh token sessions for token rotation & revocation   | Masked (IP, UA)      | 30 days after expiry/revocation  | 0.7           |
| `AuditLog`       | `audit`  | Append-only event and PII access history with before/after diff | Masked (IP, UA)      | 10 years (statutory requirement) | 0.8           |
